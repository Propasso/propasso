...
import DiagnoseLeadForm from "./DiagnoseLeadForm";
import { cn } from "@/lib/utils";

interface DiagnoseResultsProps {
...
// ---------------------------------------------------------------------------
// Tips section (shown after lead capture)
// ---------------------------------------------------------------------------

function TipsSection({ scores }: { scores: DiagnoseScores }) {
  const dimensions: Exclude<QuestionCategory, 'snapshot'>[] = [
    'attractiveness',
    'readiness',
    'owner',
  ];
  ...
  return (
    <div className="mt-12 space-y-10">
      <h3 className="font-serif text-2xl text-foreground text-center">
        Uw persoonlijke verbeterpunten
      </h3>
      {dimensions.map((dim) => {
...
const DiagnoseResults = ({ scores, snapshot, answers }: DiagnoseResultsProps) => {
    ...
    <section className="py-12 md:py-20">
      <div className="container max-w-3xl mx-auto px-4">
        ...
        <div className="space-y-5 mb-10">
          <SubScoreBar
            label={dimensionLabels.attractiveness}
            score={scores.attractiveness}
            delay={400}
          />
          <SubScoreBar
            label={dimensionLabels.readiness}
            score={scores.readiness}
            delay={600}
          />
          <SubScoreBar
            label={dimensionLabels.owner}
            score={scores.owner}
            delay={800}
          />
        </div>
        ...
        {!showTips && (
          <p className="text-xs text-muted-foreground text-center mt-8">
            U kunt uw persoonlijk rapport later alsnog opvragen via propasso.nl/diagnose
          </p>
        )}
      </div>
    </section>
